...
import { useComponentList, useCreateComponent, useUpdateComponent, useDeleteComponent, useEquipmentList, useCreateEquipment, useUpdateEquipment, useDeleteEquipment, useCategoryList } from "@/hooks/useCatalogues";
...
const MantenimientoModule = () => {
  ...
  const [registroActual, setRegistroActual] = useState<any>({ item_cod: "", item_na: "", category_id: "" });
  ...
  const handleNuevo = (tipo: ItemType) => {
    ...
    setCurrentType(tipo);
    setRegistroActual({ item_cod: "", item_na: "", category_id: "" });
    setDialogOpen(true);
  };
      ...
      <Card><CardContent className="p-0">
        <Table>
          <TableHeader><TableRow>
            <TableHead>Código</TableHead><TableHead>Nombre</TableHead><TableHead>Tipo</TableHead><TableHead>Categoría</TableHead><TableHead className="text-right">Acciones</TableHead>
          </TableRow></TableHeader>
          <TableBody>
            {filtered.map((reg: any) => (
              <TableRow key={`${reg.tipo}-${reg.item_id}`}>
                ...
                <TableCell className="text-muted-foreground">{reg.category_id ?? "—"}</TableCell>
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    <Button size="sm" variant="outline" onClick={() => handleEditar(reg)}><Pencil className="w-3 h-3 mr-1" /> Editar</Button>
                    <Button size="sm" variant="outline" className="text-destructive hover:text-destructive" onClick={() => handleEliminar(reg)}><Trash2 className="w-3 h-3" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
